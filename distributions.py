...
def _normal_ppf(p: float) -> float:
    ...
    Max absolute error < 1.15e-9 across the full (0, 1) domain.
    Reference: Peter J. Acklam, 2002.
    """
    if p <= 0.0:
        return float('-inf')
    if p >= 1.0:
...
class Poisson(Distribution):
    ...
    def pmf(self, k: int) -> float:
        ...
        if k < 0:
            return 0.0
        # Log-space to avoid overflow for large k (factorial overflows at k > ~170)
        return math.exp(k * math.log(self.lam) - self.lam - math.lgamma(k + 1))
...
class Normal(Distribution):
    ...
    def quantile(self, p: float) -> float:
        ...
        if not 0 < p < 1:
            raise ValueError("p must be in (0, 1)")
        return self.mu + self.sigma * _normal_ppf(p)
